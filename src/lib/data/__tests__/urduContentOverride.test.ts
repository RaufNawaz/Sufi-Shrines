...
const row: ShrineRow = makeShrineRow({
...
describe('mergeUrduContent', () => {
  ...
  it('is a no-op for a shrine with no override entry', () => {
    const other: ShrineRow = makeShrineRow({ Name: 'Some Other Shrine', Latitude: '1', Longitude: '1' });
  ...
  it('matches slugs derived the same way buildShrines() would (case/punctuation-insensitive)', () => {
    const withSpacing: ShrineRow = makeShrineRow({ Name: '  Data   Darbar ', Latitude: '1', Longitude: '1' });
