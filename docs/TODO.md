...
2. **Four patches await import**, and they want importing together, before that `data:build`:
   `patch_schema_hygiene_2026-08-27.csv` (off-schema categories),
   `patch_year_built_precision_2026-08-29.csv`, `patch_site_type_2026-08-30.csv`, and
   `patch_location_hygiene_2026-08-30.csv`.

   The last is the one to read before importing. **Four entries publish an instruction addressed
   to a colleague in the reader-facing `Location` field** — "ask Saifullah for a precise pin when
   possible", one opening with the literal token `FLAG:` — where it is the dateline under the
   shrine's title, a row in the infobox, and a line read aloud in the map's screen-reader
   directory. Ruled 30 August: move the instruction to `qa_note`, keep the caveat, lose no words.
   Generated by `scripts/data/build-location-hygiene-patch.mjs`, which cuts at the first `FLAG:`
   or `Ask <Name>` and can be re-run; `--check` fails while the live sheet still carries any.
   Where the mechanical cut leaves a reader worse off, move a clause back by hand — the text is
   in `qa_note`, not deleted.
