...
- ~~Row 106 "Jagannath Temple, Sialkot" ≈ row 72 "Shahwala Teja Singh Mandir" — possible duplicate.~~ RESOLVED 2026-07-10: re-researched; the Wikipedia redirect connecting the two is unsourced and the underlying fact-sets don't overlap (different locality, different founding story — see `TODO.md` §1 and `EXECUTION_PLAN.md` M1 for the full writeup). Treated as genuinely distinct; both rows now have their own descriptions and specific Location cells.
