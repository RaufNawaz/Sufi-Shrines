...
**How it's triggered:** manually, via the "Shrines enrichment (manual)" scheduled task, or by telling Claude _"run the shrines enrichment."_
...
**Working files:** the queue, log, and other scratch files this runbook mentions
(`_enrichment_queue.md`, `_ENRICHMENT_LOG.md`, `_image_unverified.md`,
`_enrichment_batch.md`, `_staging_descriptions.md`) live under **`archive/`** at the
repo root, alongside `archive/xlsx_backups/`. That directory is **gitignored** — like
the workbook itself, these files exist only on the maintainer's machine and are not
followable from a clean clone. `shrines_enrich.py` reads/writes
`archive/_enrichment_queue.md` and `archive/_ENRICHMENT_LOG.md` directly.
...
This prints: data-row count, rows missing a Description, rows missing Image 1, and the next candidate sites from `archive/_enrichment_queue.md` (flagging any already in the sheet).
...
- **4–6 new shrines** from the top of `archive/_enrichment_queue.md` (verify each is real and not already present before writing).
...
- Warm, dignified, scholarly-narrative prose. **British spellings** (centre, honour, defence). Italicise transliterated terms/titles on first use with markdown `*...*` (e.g. _urs_, _langar_, _sarovar_, _Shakti Peetha_, _Guru Granth Sahib_).
...
- Read an existing long entry first for tone (e.g. row 2 Data Darbar, or `archive/_staging_descriptions.md` from the first session).
...
- **Flag auto-sourced images.** When a file is accepted on filename match but not actually viewed, list it in `archive/_image_unverified.md` (row, site, field, Commons file, a `[ ]` checkbox) and note in `archive/_ENRICHMENT_LOG.md` that the batch is unverified, so a human or a later browser-enabled run can spot-check.
...
Create `archive/_enrichment_batch.md` containing your blocks in these EXACT formats:
...
```
python3 tools/shrines_enrich.py --write archive/_enrichment_batch.md
```
...
- In `archive/_enrichment_queue.md`: change `- [ ]` to `- [x]` for sites you added, and append any new candidate sites you discovered.
...
- Every run is appended to `archive/_ENRICHMENT_LOG.md`.

## Known follow-ups (from the 2026-07-04 session)

- ~~Row 6 (Ganj-e-Inayat): Image 1 is a Facebook page URL, not a photo — replace or clear.~~ RESOLVED 2026-07-10: cleared (no valid Commons replacement found after a fresh search — left empty rather than fabricated).
- **Row 106 "Jagannath Temple, Sialkot" ≈ row 72 "Shahwala Teja Singh Mandir" — CONFIRMED DUPLICATE, needs a human decision.** Web research 2026-07-10: press coverage of the row-106 temple (built 2007, Paris Road, funded via a CM Punjab grant) states it "was also known as Shawala Teja Singh Temple," and Wikipedia's `Jagannath_Temple,_Sialkot` URL redirects to its `Shivala Teja Singh temple` article — i.e. these are two spreadsheet rows for the same physical building. The two rows carry slightly different coordinates (32.4908,74.5449 vs 32.495833,74.535 — ~700m apart, likely one imprecise pin) and different deity framing (Shiva vs Jagannath/Krishna) — possibly the temple's identity/dedication itself is disputed or changed over time, which is worth a targeted read of both descriptions before merging. This needs a human call (which row to keep/merge) rather than an automated delete, since it changes the shrine count.
- 3 unverified image candidates for rows 96 / 104 / 106 mentioned in a prior `_RESUME_LOG.md` — that file no longer exists in this checkout (likely cleaned up); could not re-locate the specific candidates it referred to.
