...
| [`planning/DECISION_figure_identity_column.md`](planning/DECISION_figure_identity_column.md) | **Decision required:** the graph builds figure identity from the legacy `Sufi Saint` column, not `principal_figure`. 47 rows and 44 of 132 figure slugs would move. The composite half is closed (fan out); the column is now a 169-row worksheet — `npm run data:review:figures`, 15 rows genuinely contested |
