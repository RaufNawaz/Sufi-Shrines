...
| [`planning/DECISION_figure_identity_column.md`](planning/DECISION_figure_identity_column.md) | **Decision required:** the graph builds figure identity from the legacy `Sufi Saint` column, not `principal_figure`. 50 rows and 47 of 132 figure slugs would move; the composites that name two Gurus wait on the same call |
