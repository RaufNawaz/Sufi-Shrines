...
### Added 29 August 2026 — the font shift is real, it is the sans stack, and it was never on the route we were watching

The 29 August sweep attributed `/order/qadiriyya`'s CLS 0.2186 to `sourcesans3…woff2` and
`merriweather…woff2`, and the entry above concluded the fix was "`font-display`/metric-matching
or the self-hosted fonts". Metric-matching is now done — eight `@font-face` rules in
`global.css`, numbers in `data/font-metrics.json`, generated by
`scripts/measure-font-metrics.mjs`. Four things came out of doing it, and three of them
correct something this file already said.

#### 1. `/order/qadiriyya` did not reproduce, and the shift was on a route nobody had looked at

Lighthouse on `/order/qadiriyya` today, production build, three runs each way:
**baseline 0 / 0.0002 / 0.0002, fixed 0 / 0 / 0.0001.** The 0.2186 was not there to remove. That
is the third reading of this route — 0.0002 on 28 August, 0.2186 on 29 August, ~0 now — and it
settles the earlier entry's question in its favour: *the Lighthouse figure for this route is a
coin toss on whether the font lands before paint*, and no amount of re-running it produces a
number worth acting on.

So stop leaving the timing to chance. `scripts/measure-font-swap-shift.mjs` **holds the
`fonts.gstatic.com` responses back for a fixed delay**, so the swap is always late, and runs both
token stacks against one build with `!important` custom properties as the only variable. It is
deterministic where Lighthouse is not: five runs, five identical figures.

What it found, cold cache, fonts held 1500ms, three runs each, all identical:

| route | baseline | with the faces |
|---|---|---|
| `/saint/data-ganj-bakhsh` | **0.2058** | **0.0001** |
| `/chronology` | 0.0171 | 0.0000 |
| `/order/qadiriyya` | 0.0002 | 0.0001 |
| `/shrine/data-darbar` | 0.0004 | **0.0048** |

**The archive's font shift is 0.2058 and it lives on `/saint/data-ganj-bakhsh`.** The sweep found
it on `/order` because that is where the timing happened to land that night, and the eleven-route
list — the thing that was supposed to stop us measuring two pages of thirteen — still only sees
a route on the run where it misbehaves. The instrument now names the movers:
`div.entity-article-layout + a.entity-urs-link + li.entity-title-chip + span.almanac-flag`.

#### 2. It is the sans stack. The article prose is `--font-sans`, not `--font-serif`

This is the non-obvious fact that cost the most time. `--font-serif` is headings; the article
body is `--font-sans`. Every large number here belongs to Source Sans 3, and a change that
adjusts only the serif fallbacks moves nothing.

#### 3. A measurement that was true and misleading, which is why it is written down

**`system-ui` on macOS measures within 0.08% of Source Sans 3** on mean advance width. Read
straight, that says SF Pro is already a near-perfect stand-in and inserting an adjusted Helvetica
face ahead of it can only do harm — and one observation agreed: it put a badge row on
`/shrine/data-darbar` 37px from where the webfont puts it, where SF Pro had it exactly right. The
four sans faces were dropped on that basis.

Then the *article* was measured, with the webfont blocked outright — which is not a hypothetical
for this archive, since `index.html`'s own comment says the CDN is "periodically slow or
unreachable" for readers in Pakistan, and for them the fallback is the whole visit:

| route | designed | SF Pro (baseline) | adjusted face |
|---|---|---|---|
| `/shrine/data-darbar` | 16394 | 17350 (**+956**) | 16369 (−25) |
| `/about` | 64284 | 64803 (+519) | 64331 (+47) |
| `/chronology` | 7125 | 7618 (+493) | 7171 (+46) |
| `/order/qadiriyya` | 16320 | 16779 (+459) | 16321 (**+1**) |

**A mean advance over one sentence does not predict where lines break.** Two faces can agree to a
twelfth of a percent and still wrap a 16,000px article 956px differently. The faces went back in.
The 0.08% is kept in `data/font-metrics.json` under `reference` because the number is correct and
the inference from it was not.

**The cost, stated rather than buried:** `/shrine/data-darbar` goes 0.0004 → 0.0048 (attributed:
`span.info-level-badge + span.support-level-badge`), and that badge row stays 37px out. Against
0.2058 removed and ~950px of drift, it is the right trade, and it is not free.

#### 4. Two instrument bugs the self-check caught, and one corruption nothing explains

`measure-font-metrics.mjs` designs each fallback face, then **declares it and re-measures it**
against the webfont it imitates. That check earned its place twice before a single number shipped:

- The Google Fonts `css2` response carries **one `@font-face` per subset, and the first is
  Cyrillic Extended.** Taking the first `url()` gave a face with no Latin glyphs, so the sample
  fell back to the platform serif while the line box still came from the declared face — a row
  averaging two fonts, reading as plausible. Tinos and Arimo now measure byte-identically to Times
  New Roman and Arial, which is what metric-compatible families are *supposed* to do and is the
  independent check that the numbers are right.
- The self-check probe for a CDN-measured face used `local()`, which by definition cannot resolve,
  so it silently measured the default font.

Separately, and **unexplained**: partway through this work all three self-hosted Noto Nastaliq
Urdu faces in `global.css` were found carrying the *Latin* `unicode-range` instead of their
Arabic one. No command in the session reproduces it, insertion and prettier were both cleared by
experiment, and nothing reached a commit — the working tree was shared with another agent at the
time. The blast radius is what matters: a Nastaliq face restricted to Latin is never selected for
Arabic script, so the whole Urdu edition silently loses its reading face. Nothing throws, no
English route changes, and **the e2e no-leak guard would not fire either**, because the text is
still Urdu — only the typeface is wrong. `fontFallbackMetrics.test.ts` now asserts all three faces
exist and each still covers U+0600–06FF. RULE 4: a class of bug that has happened once gets an
invariant, whether or not the cause was ever found.

#### What did not change, and why

**The CLS budget stays `warn`.** Yesterday's entry deferred promoting it on `/order/qadiriyya`'s
0.219; that route is now ~0 in Lighthouse and 0.0001 in the deterministic instrument. But the
reason to keep it is unchanged and better evidenced: the Lighthouse number for a font-sensitive
route is decided by whether the font beat the paint, and an assertion that is red or green by luck
is worse than no assertion. `measure-font-swap-shift.mjs` is the thing to gate on if anything is,
because it is reproducible — but it needs a route list chosen from where the shift actually is,
which is now known to be `/saint`, not `/order`.

**Two faces are unverified on the device.** Roboto and Liberation Sans/Serif are not installed on
