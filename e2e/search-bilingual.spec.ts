...
async function search(page: Page, query: string) {
  ...
  // The worker is debounced; wait for the count to settle rather than a fixed
  // delay, so this neither flakes on a slow machine nor sleeps on a fast one.
  await expect
    .poll(async () => page.locator('.shrine-list-item').count(), { timeout: 8000 })
    .toBeGreaterThan(-1);
  await page.waitForTimeout(700);
  return page.locator('.shrine-list-item').count();
}
